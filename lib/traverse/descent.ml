...
(* Descent for Pstructrures *)
module Str = struct

  let d_eval iter e attrs = 
    iter.attributes iter attrs;
    iter.expr iter e

  let d_value iter vbindings = 
    List.iter (iter.value_binding iter) vbindings

  let d_type iter tdecls = 
    List.iter (iter.type_declaration iter) tdecls

  let d_primitive iter vd = iter.value_description iter vd

  let d_typext iter te = iter.type_extension iter te
end